...
async function getPopulars(_API_KEY, page, dateGte, dateLte){
    return await fetch(`${_baseUrl}/discover/movie?api_key=${_API_KEY}&page=${page}&`)
    .then(response => response.json())
    .then(data => {
        // console.log(data);
        return data.results;
...
async function main(){
    ...
    const page = 2;
    //const data = await getPopulars(_API_KEY, page);
    //console.log(data);
    //displayMovies(data);
